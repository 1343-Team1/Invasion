Shader "Custom/Translucent" {
	Properties{
		...
		_IgnoreBelowR("Ignore Below (R)", Float) = 50
		_IgnoreBelowG("Ignore Below (G)", Float) = 50
		_IgnoreBelowB("Ignore Below (B)", Float) = 50
	}
		SubShader{
	...
	float _Scale, _Power, _Distortion, _IgnoreBelowR, _IgnoreBelowG, _IgnoreBelowB;
	...
	inline fixed4 LightingTranslucent(SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
	{
		...
		if (c.r < _IgnoreBelowR && c.g < _IgnoreBelowG && c.b < _IgnoreBelowB) return c;
